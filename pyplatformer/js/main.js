...
const codeKey = (id) => `pyplatformer.code.${id}`;
function loadSavedCode(level) {
  try {
    const v = localStorage.getItem(codeKey(level.id));
    return v !== null ? v : (level.starterCode || "");
  } catch { return level.starterCode || ""; }
}
function saveCode(level, code) {
  try { localStorage.setItem(codeKey(level.id), code); } catch {}
}
...
function populateLevelSelect() {
  ...
  for (const entry of manifest) {
    ...
    const KIND_LABELS = {
      demo: " (demo)", activity: " (activity)", homework: " (homework)",
    };
...
function renderBrief(level) {
  ...
  if (level.api && level.api.length) {
    apiRefBody.innerHTML = "<ul>" +
      level.api.map(n => `<li><code>${n}</code></li>`).join("") + "</ul>";
  } else {
    apiRefBody.innerHTML = "<p class='sys'>All commands available.</p>";
  }
  if (level.solution) {
    solutionBody.textContent = level.solution;
    solutionDisclosure.hidden = false;
    solutionDisclosure.open = false;
  } else {
...
async function activateLevel(id, { keepCode = false } = {}) {
  ...
  try {
    ...
    renderBrief(level);
    if (!keepCode) editor.setValue(loadSavedCode(level));
...
async function onRun() {
  ...
  if (result.aborted)  { setStatus("Stopped"); return; }
  if (result.error)    { setStatus("Error", "error"); return; }
  if (result.win) {
    ...
    let body;
    if (wasFirstTime && next)        body = "Nicely done. Ready for the next one?";
    else if (wasFirstTime && !next)  body = "Nicely done — that was the last level!";
    else if (!wasFirstTime && next)  body = "Solved again. Try another approach?";
    else                             body = "Solved again — and that was the last level!";
    showOverlay("Level Complete!", body, { hasNext: !!next });
  } else {
    setStatus("Try again", "error");
    showOverlay("Not quite", "Your code ran without errors, but didn't collect the flag.", { hasNext: false });
  }
}
...
const SCALE_KEY = "pyplatformer.scale";
...
scaleSelect.addEventListener("change", (e) => applyScale(parseInt(e.target.value, 10)));
levelSelect.addEventListener("change", (e) => activateLevel(e.target.value));
...
(async () => {
  ...
  catch (e) { setStatus("Failed to load levels", "error"); console.error(e); return; }
  populateLevelSelect();
  await activateLevel(manifest[0].id);

  runner.load().then(() => {
    setStatus("Ready");
    runBtn.disabled = false;
    runBtn.textContent = "▶ Run (Ctrl+Enter)";
  }).catch((e) => setStatus("Failed to load Python: " + e.message, "error"));
})();
